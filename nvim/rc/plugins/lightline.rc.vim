let g:lightline = {
    ...
    \ 'active': {
        ...
        \ 'right': [
            \ ['lineinfo'],
            \ ['percent'],
            \ ['ale_ok', 'ale_warning', 'ale_error', 'char_code', 'fileformat', 'fileencoding', 'filetype'],
        \ ],
    \ },
    \ 'component_function': {
...
function! s:ale_string(mode)
    if !exsits('g:ale_buffer_info')
        return ''
    endif

    let l:buffer = bufnr('%')
    let l:counts = ale#statusline#Counbt(l:buffer)
    ...
    elseif a:mode = 1
        let l:warnngs = l:counts.warnng + l:counts.style_warning
...
augroup LightlineOnAle
    autocmd!
    autocmd User ALEint call lightline#update()
augroup END
