...
- Do not write code comments explaining what the code does or why the change was made. The only admissible comment is a why-not: why a seemingly natural alternative would break, and only when that is not recoverable from the code, commit history, or upstream docs.
